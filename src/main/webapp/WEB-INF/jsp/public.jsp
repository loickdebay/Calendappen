...
<header>
        <div>
            ...
            <a href ='login'>
            <button class='greenbutton'>Connexion</button>
            </a>
            <a href="login">
...
while (ldCal.getMonthValue() == month) {
    ...
    if(!localDateCheck.isBefore(LocalDate.now())){
        if(exeptionalWorkDays!=null && exeptionalWorkDays.contains(targetWorkDay)){
            out.println("<form action='publicReservation' method='get'>");
            out.println("<input type='hidden' name='id' value='" + cellId + "'>");
            out.println("<button type='submit'>Accès</button>");
            out.println("</form>");
        }
        if(closedDays != null && closedDays.contains(targetCloseDay) ){
            ...
            if(workdays.contains(ldCal.getDayOfWeek().name().toLowerCase())){
                out.println("<form action='publicReservation' method='get'>");
                out.println("<input type='hidden' name='id' value='" + cellId + "'>");
                out.println("<button type='submit'>Accès</button>");
                out.println("</form>");
...
<div class="button-container">
    <a href="public?mois=<%= localDate.minusMonths(1).getMonthValue() %>&annee=<%= localDate.minusMonths(1).getYear() %>">
        <button>Précédent</button>
    </a>
    <a href="public?mois=<%= LocalDate.now().getMonthValue() %>&annee=<%= LocalDate.now().getYear() %>">
        <button>Mois actuel</button>
    </a>
    <a href="public?mois=<%= localDate.plusMonths(1).getMonthValue() %>&annee=<%= localDate.plusMonths(1).getYear() %>">
